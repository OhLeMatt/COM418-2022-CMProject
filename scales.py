...
class Scale:
    def __init__(self, tonic_chroma, shifts):
...
def chromatic_scale():
    return Scale(0, CHROMATIC_SHIFTS)
...
def ditetrachordic_scale(tonic_chroma, **kwargs):
    return Scale(tonic_chroma, ditetrachordic_shifts(**kwargs))
...
def diatonic_scale(tonic_chroma, mode=Mode.MAJOR):    
    return Scale(tonic_chroma=tonic_chroma, shifts=diatonic_shifts(mode))
...
def gipsy_major_scale(tonic_chroma):
    return Scale(tonic_chroma=tonic_chroma, shifts=GIPSY_MAJOR_SHIFTS)

def neapolitan_minor_scale(tonic_chroma):
    return Scale(tonic_chroma=tonic_chroma, shifts=NEAPOLITAN_MINOR_SHIFTS)

def altered_scale(tonic_chroma):
    return Scale(tonic_chroma=tonic_chroma, shifts=ALTERED_SHIFTS)
